...
const char* ArtMethod::GetTraceMd(){
    return packageConfig.traceMethod;

}

const char* ArtMethod::GetDebugMd(){

    return packageConfig.debugMethod;
}

bool ArtMethod::IsTuo(){
    return packageConfig.isTuoke;
}

bool ArtMethod::IsDeepInvk(){
    return packageConfig.isDeep;
}

bool ArtMethod::IsRegNativePrint(){
    return packageConfig.isRegisterNativePrint;
}

bool ArtMethod::IsJNIMdPrint(){
    return packageConfig.isJNIMethodPrint;
}

bool ArtMethod::IsInvkPrint(){
    return packageConfig.isInvokePrint;
}

char* ArtMethod::GetPkgName(){
    return packageConfig.packageName;
}


void ArtMethod::SetPkgItem(JNIEnv* env,jobject config){
    LOG(ERROR)<< "godrom ArtMethod SetPkgItem enter";
    //获取Java中的实例类ParamInfo
    jclass jcInfo = env->FindClass("cn/god/PackageItem");
    //获取类中每一个变量的定义
    //boolean boolValue
    ...
    oss << "godrom SetPkgItem isDeep:"<<packageConfig.isDeep<<" debugMethod:"<<packageConfig.debugMethod<<
...
//dumpdexfilebyCookie
extern "C" void duppDexOver()  REQUIRES_SHARED(Locks::mutator_lock_) {
    if(dex_map.size()<=0){
    		LOG(ERROR) << "godrom duppDexOver dex_map.size()<=0";
        return;
    }
    char *dexfilepath=(char*)malloc(sizeof(char)*1000);
    LOG(ERROR) << "godrom ArtMethod::duppDexOver";
    int result=0;
    char* packageName=ArtMethod::GetPkgName();
    ...
    for(iter = dex_map.begin(); iter != dex_map.end(); iter++) {
        ...
        if(dexfilefp>0){
          ...
          if(fp>0)
          {
              ...
              if(result<0)
              {
                  LOG(ERROR) << "godrom ArtMethod::duppDexOver,open dexfilepath error";
              }
              fsync(fp);
              close(fp);
              memset(dexfilepath,0,1000);
	...
	//在函数即将调用解释器执行前进行dump。
extern "C" void duppdexfilebyExecute(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
    if(artmethod==nullptr){
        LOG(ERROR)<< "godrom ArtMethod::duppdexfilebyExecute artmethod is null";
        return;
    }
    char *dexfilepath=(char*)malloc(sizeof(char)*1000);
    if(dexfilepath==nullptr)
    {
    		std::ostringstream oss;
    		oss<<"godrom ArtMethod::dumpdexfilebyArtMethod,methodname:"<<artmethod->PrettyMethod().c_str()<<"malloc 1000 byte failed";
        LOG(ERROR)<< oss.str();
        return;
    }
    int result=0;
    ...
    if(dex_file==nullptr){
        LOG(ERROR)<< "godrom ArtMethod::duppdexfilebyExecute dex_file is null";
        return;
    }
    ...
    if(dexfilefp>0){
      ...
      dexfilefp=0;
              ...
              if(fp>0)
              {
                  ...
                  if(result<0)
                  {
                    LOG(ERROR) << "godrom ArtMethod::dumpdexfilebyArtMethod,open dexfilepath error";
                  }
                  fsync(fp);
                  close(fp);
                  memset(dexfilepath,0,1000);
                  ...
                  if(classlistfile>0)
                  {
                        for (size_t ii= 0; ii< dex_file->NumClassDefs(); ++ii)
                        {
                            ...
                            if(result<0)
                            {
                                LOG(ERROR) << "godrom ArtMethod::duppdexfilebyExecute,write classlistfile file error";
                            }
                            const char* temp="\n";
...
//主动调用函数的dump处理
extern "C" void duppArtMethod(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
            //LOG(ERROR) << "godrom ArtMethod::duppArtMethod "<<artmethod->PrettyMethod().c_str();
			char *dexfilepath=(char*)malloc(sizeof(char)*1000);
			if(dexfilepath==nullptr)
			{
				std::ostringstream oss;
				oss<<"godrom ArtMethod::duppArtMethodinvoked,methodname:"<<artmethod->PrettyMethod().c_str()<<"malloc 1000 byte failed";
				LOG(ERROR) << oss.str();
				return;
			}
			int result=0;
			char* packageName=ArtMethod::GetPkgName();
				...
				if(dexfilefp>0){
					...
					LOG(ERROR) << "godrom ArtMethod::dumpdexfilebyArtMethod save dex_map";
					...
					if(fp>0)
					{
							...
							if(result<0)
							{
								 LOG(ERROR) << "godrom ArtMethod::dumpdexfilebyArtMethod,open dexfilepath file error";
							}
							fsync(fp);
							close(fp);
							memset(dexfilepath,0,1000);
							...
							if(classlistfile>0)
							{
										for (size_t ii= 0; ii< dex_file->NumClassDefs(); ++ii)
										{
												...
												if(result<0)
												{
														LOG(ERROR) << "godrom ArtMethod::dumpdexfilebyArtMethod,write classlistfile file error";
				...
				{
							//LOG(ERROR) << "godrom duppArtMethodinvoked,methodname:"<<artmethod->PrettyMethod().c_str()<<" write bin";
							int code_item_len = 0;
							...
							if(fp2>0){
									...
									if(result<0)
									{
											LOG(ERROR) << "godrom ArtMethod::dumpdexfilebyArtMethod,write ins file error";
									}
									long outlen=0;
			...
			//LOG(ERROR) << "godrom ArtMethod::duppArtMethod over "<<artmethod->PrettyMethod().c_str();
}
extern "C" void fatrextInvoke(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  //add
		if ((result!=nullptr && result->GetI()==888888)){
			if(!ArtMethod::IsDeepInvk()){
					duppArtMethod(this);
					return;
			}
		...
		//else{
		//	if(ArtMethod::IsInvkPrint()){
		//				std::ostringstream oss;
		//				oss<<"godrom art invoke "<<this->PrettyMethod().c_str();
		//				LOG(ERROR) <<oss.str();
		//		}
		//}
	//add end
  ...
  if ((result!=nullptr && result->GetI()==888888)&&!IsNative()){
  ...
  if (UNLIKELY(!runtime->IsStarted() ||
    ...
    if (IsStatic()) {
  	...
  	if (result!=nullptr && result->GetI()==888888){
  			std::ostringstream oss;
				oss<< "godrom artMethod::Invoke return Native Method "<<this->PrettyMethod().c_str();
				LOG(ERROR) <<oss.str();
				return;
		}
...
const void* ArtMethod::RegisterNative(const void* native_method) {
  ...
  if(ArtMethod::IsRegNativePrint()){
        std::ostringstream oss;
        oss << "godrom RomPrint RegisterNative name:"<<this->PrettyMethod().c_str()<<" native_ptr:"<<native_method<<" method_idx:"<<this->method_index_;
    		LOG(ERROR)<<oss.str();
    }
