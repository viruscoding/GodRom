...
class ArgArray {
 public:
  ...
  void VarArgsShowArg(const ScopedObjectAccessAlreadyRunnable& soa,
        ...
        REQUIRES_SHARED(Locks::mutator_lock_) {
      std::stringstream ss;
		  ss << "godrom jni "<<jniMethodName<<"\t"<<methodName<<"\t";
      for (size_t i = 1; i < shorty_len_; ++i) {
        switch (shorty_[i]) {
          case 'Z':
          case 'B':
          ...
          case 'L':{
          	jobject obj=va_arg(ap, jobject);
						...
						if(receiver==nullptr){
							LOG(ERROR)<<"godrom  "<<jniMethodName<<" receiver =nullptr";
							break;
						}
...
void InvokeWithArgArray(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if(ArtMethod::IsInvkPrint()){
		...
		if(artMethod!= nullptr) {
				...
				oss << "godrom [InvokeWithArgArray]beforecall caller:" << artMethod->PrettyMethod() << "---called:"<< method->PrettyMethod();
				LOG(ERROR)<<oss.str();
		}
	}
...
void ShowJniField(const char* jniMethodName,jfieldID field) {
		if(ArtMethod::IsJNIMdPrint()){
			...
			std::ostringstream oss;
			oss << "godrom jni "<<jniMethodName<<"\t"<<ArtField::PrettyField(f);
			LOG(ERROR)<<oss.str();
		}
  }
...
void ShowArgStr(const char* jniMethod,const char* str1,const char* str2){
	if(ArtMethod::IsJNIMdPrint()){
			...
			if(str1!=nullptr && str2!=nullptr){
				oss << "godrom jni "<<jniMethod<<"\t"<<str1<<"\t"<<str2;
			}else if(str1!=nullptr ){
				oss << "godrom jni "<<jniMethod<<"\t"<<str1;
			}else{
				oss << "godrom jni "<<jniMethod;
			}
			LOG(ERROR)<<oss.str();
  }
}
