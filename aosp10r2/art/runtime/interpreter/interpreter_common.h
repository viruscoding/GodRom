...
static inline void TraceExecution(const ShadowFrame& shadow_frame, const Instruction* inst,
    ...
    REQUIRES_SHARED(Locks::mutator_lock_) {
    //strstr(ArtMethod::GetTraceMd(),shadow_frame.GetMethod()->PrettyMethod().c_str())

    if(shadow_frame.GetMethod()!=nullptr && ArtMethod::GetTraceMd()!=nullptr&&strlen(ArtMethod::GetTraceMd())>0){
        const char* methodName=shadow_frame.GetMethod()->PrettyMethod().c_str();
        if(strstr(methodName,ArtMethod::GetTraceMd())) {
        	std::ostringstream oss;
					oss << android::base::StringPrintf("godrom smaliTrace 0x%x: ", dex_pc)
