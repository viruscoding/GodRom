...
//     }
//add
extern "C" void duppdexfilebyExecute(ArtMethod* artmethod);
//addend
...
static inline JValue Execute(
  ...
  if((result_register.GetI()==888888)){
    return ExecuteSwitchImpl<false, false>(self, accessor, shadow_frame, result_register,false);
  }
  if(ArtMethod::GetTraceMd()!=nullptr && strlen(ArtMethod::GetTraceMd())>0){

			if(strstr(method->PrettyMethod().c_str(),ArtMethod::GetTraceMd())){
				std::ostringstream oss;
				oss<< "godrom Execute strstr:"<<method->PrettyMethod().c_str()<<"\t"<<ArtMethod::GetTraceMd();
				LOG(ERROR) <<oss.str();
				return ExecuteSwitchImpl<false, false>(self, accessor, shadow_frame, result_register,false);
			}
  }
...
void EnterInterpreterFromInvoke(Thread* self,
  ...
  if (LIKELY(!method->IsNative())) {
    ...
    if(result!=nullptr&&result->GetI()==888888){
        ...
        //LOG(ERROR) << "fatrext Execute over"<<method->PrettyMethod().c_str();
        return;
    }else{
