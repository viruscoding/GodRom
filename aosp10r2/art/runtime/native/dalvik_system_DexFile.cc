...
namespace art {
//add
extern "C" void fatrextInvoke(ArtMethod* artmethod);
extern "C" ArtMethod* jobjectToArtMethod(JNIEnv* env, jobject javaMethod);
extern "C" void duppDexOver();
...
static jboolean DexFile_setGodRomConfig(JNIEnv* env,jclass,jobject config){
    ArtMethod::SetPkgItem(env,config);
    return JNI_TRUE;
}

//addfunction 将ava的Method转换成ArtMethod。然后主动调用
static void DexFile_fatrextMethodCode(JNIEnv* env, jclass,jobject method) {
  if(method!=nullptr)
  {
      ArtMethod* proxy_method = jobjectToArtMethod(env, method);
      fatrextInvoke(proxy_method);
  }
  return;
}

static void DexFile_duppRepair(JNIEnv* env, jclass){
    if(env==nullptr){
        return;
    }
    duppDexOver();
...
static JNINativeMethod gMethods[] = {
  ...
  //add
  NATIVE_METHOD(DexFile, fatrextMethodCode,"(Ljava/lang/Object;)V"),
  NATIVE_METHOD(DexFile, duppRepair,"()V"),
  NATIVE_METHOD(DexFile, setGodRomConfig,"(Ljava/lang/Object;)Z"),
