...
extern uint32_t JniMethodStart(Thread* self) {
  ...
  if(ArtMethod::GetDebugMd()!=nullptr && strlen(ArtMethod::GetDebugMd())>0){
    LOG(ERROR)<< "godrom JniMethodStart strstr methodname:"<<methodname<<" debugMethod:"<<ArtMethod::GetDebugMd();
    if(strstr(methodname.c_str(),ArtMethod::GetDebugMd())!=nullptr){
            LOG(ERROR)<< "godrom JniMethodStart methodname:"<<methodname<<" wait debug sleep 60...";
            sleep(60);
        }
  }
